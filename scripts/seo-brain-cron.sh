...
2. 每天最多改 5 個檔，寧少勿多。
3. 改完自我驗證：pnpm install --frozen-lockfile → pnpm build:changed（依變更檔案挑來源 gate；改到 .astro 文案會跑文案檢查）。任一非零 → git checkout . 撤回、今日不 push、走步驟6發 Slack 標🔴回報。
...
- pnpm install --frozen-lockfile → pnpm build:changed。任一失敗 → git checkout .、不 push、跳步驟5（Slack 標🔴）。
- 有內容改動且 DRY_RUN=0：
