#!/usr/bin/env node
// 節慶站內戰役 gate：排程只能定義在 src/lib/seasonal-campaigns.ts，
// 且每個 CTA 目標都必須存在於 festivals.json，並有資料關係或逐句掛源內容可供驗證。
...
const EXPECTED = [
  'guimenkai', 'qixi', 'fangshuideng', 'zhongyuan', 'dizang',
  'september-solar-terms', 'kinmen-bo-bing', 'zhongqiu', 'kongzi-birthday',
];
if (seasonalCampaigns.length !== EXPECTED.length) {
  errors.push(`戰役排程應為 ${EXPECTED.length} 檔，實際 ${seasonalCampaigns.length}`);
}
for (const slug of EXPECTED) {
  if (!seasonalCampaigns.some((c) => c.festivalSlug === slug)) errors.push(`戰役排程缺 ${slug}`);
}
...
for (const [i, c] of seasonalCampaigns.entries()) {
  ...
  if (c.target < c.start) {
    errors.push(`${c.festivalSlug} target ${c.target} 早於曝光起日 ${c.start}`);
  }
  const refs = ['deity_refs', 'practice_refs', 'event_refs', 'temple_refs']
    .flatMap((key) => festival[key] ?? []);
  if (refs.length === 0 && !(festival.facts ?? []).length) {
    errors.push(`${c.festivalSlug} 沒有資料關係或逐句掛源 facts`);
  }
