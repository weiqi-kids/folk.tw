...
const LEDGER = [
  ...
  },
  {
    ...
    blocker:
      ...
      '⛔ 等台灣端抓列表頁。⚠️ v18 的 expect.contains 寫成未跳脫的 `?ci=2&cid=`，' +
      '而來源逐字輸出 `?ci=2&amp;cid=` → 14 個 job 全部驗不過、一個 byte 都沒落地（台灣端 2026-08-20 回報，' +
      '拿基準檔 cid=3 複驗屬實：未跳脫 0 次、跳脫 212 次）。**v19 已修並發布**，下一輪 cron 自然會收。' +
      '同輪加了 gate `check:manifest-expect`（拿本機樣本回測每條 expect），負向測試會咬。' +
      '📌 台灣端同時驗出：分頁是假的（頁面自報「共 N 頁」但第 1 頁 HTML 就含該分類全部條目，' +
      '15 個分類逐一驗過自報筆數＝相異條目 cid 數），**一個 URL＝整個分類，不要加 paginate**；' +
      '條目 cid 與分類 cid 共用同一個數字命名空間、靠路徑區分，產 url_list 清單時不要混。' +
