...
// ═══════════════════════════════════════════════════════════════════════════
//
// 站上第一紅線是「絕不杜撰，所有事實逐筆掛源，**來源要能被機器複驗**」。
...
//
// ② **逐字引用掛到沒有授權的來源**
//    站上的「逐字引用、不改寫」內容各自綁著一次明確授權，網域用白名單制：
//    不在名單上就擋。名單與各自的條件見下方 VERBATIM_ALLOW。
//    逐字引用是**授權範圍最窄的用法**，所以這條訂得硬。
//
// ③ **未授權來源的引用數只能降不能升**（grandfather）
//    這條的對象表（UNLICENSED／BASELINE）**目前是空的**——原本列管的兩個站都已取得授權：
//      ・`taiwangods.moi.gov.tw`：2026-08-19 站主回報內政部確認同意。
//      ・`th.gov.tw`（國史館臺灣文獻館）：2026-08-19 站主確認亦已授權。
//    ⚠️ 機制刻意留著不刪：下一個「大量引用但授權未明」的來源出現時，把它加回 UNLICENSED
//    與 BASELINE 就能立刻凍住擴散。為什麼是「只能降不能升」而不是直接禁——現況常已有
//    數百處，一刀切會讓所有部署紅燈，那不是擋錯誤、那是擋工作。
//
// ④ **名錄快照的新鮮度**：只 WARN，不擋。
//    🔴 刻意不擋：gate 一旦因為「外部名錄過期」而紅燈，等於把別人的更新頻率
//    綁進站上的發佈路徑。過期的實際影響只有「新登錄的個案還不能引用」，那可以等。
//
...
const VERBATIM_ALLOW = new Set([
  ...
  // 國史館臺灣文獻館，2026-08-19 站主確認**已取得授權**（同一輪追認 taiwangods 時一併確認），
  // 條件同前＝標示資料來源連結（逐筆掛回該筆自己的原始網址），站主 2026-08-19 明確確認。
  // 🔴 **主機名要逐一列**（比照 boch 那兩個）：本名單是 exact match，`th.gov.tw` 不會涵蓋
  //    `www.th.gov.tw`／`dict.th.gov.tw`。刻意不改成後綴比對——白名單制的價值就在於
  //    「哪個站被授權過」是逐筆寫下來的，後綴比對會讓沒查過的新子網域自動獲得逐字權。
  'th.gov.tw',
  'www.th.gov.tw', // 電子報〈臺灣人、臺灣事〉等文章
  'dict.th.gov.tw', // 《臺灣民俗文物辭典》辭條
]);

// 已知授權有問題的站（規則 ③ 的計數對象，也是規則 ② 的明確拒絕理由）。
// 2026-08-19 起**兩個都已移出**：taiwangods.moi.gov.tw（站主回報內政部確認同意）與
// th.gov.tw（站主確認已授權）。grandfather 上限一併移除——那條規則的用途是
// 「未授權來源不得再擴散」，前提消失了，留著只會擋住正常使用。
// 🔴 空的不代表機制沒用：新的「大量引用但授權未明」來源出現時，加回這裡＋BASELINE 即生效。
const UNLICENSED = {};

// 🔴 grandfather 基準。**只能往下調，不可往上調。**
//    往上調等於把「又多引用了一處未授權來源」正常化——那正是這條要擋的事。
//    加新來源時，跑一次本檔看實測值再填，不要憑印象填。
const BASELINE = {};
