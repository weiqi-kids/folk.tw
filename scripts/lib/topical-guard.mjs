...
//   理由：那些數字未經機器複驗、且隨救援變動；站上只做「為平安／復原祈福」，不是災情速報。
...
const MAINLAND_TERMS = [
  // ── 第 0 層：簡繁轉換殘字（單字，2026-07-27 加；起因「受影响之地」漏過只收詞的表）──
  // **必須排在最前面**：先把字形正規化，後面的詞層才比得中（「搶险」要先變「搶險」才會被換成「搶救」）。
  // **只收在正體中該字形根本不存在、且對映唯一的簡化字**；一簡對多繁者一律不收
  //（发＝發/髮、后＝後/皇后、干＝乾/幹、云＝雲/云、台＝臺/台、里＝裡/里、面＝麵/面、尽＝盡/儘、获＝獲/穫…）。
  ['响', '響'], ['级', '級'], ['报', '報'], ['门', '門'], ['问', '問'], ['这', '這'],
  ['们', '們'], ['时', '時'], ['间', '間'], ['来', '來'], ['长', '長'], ['东', '東'],
  ['车', '車'], ['马', '馬'], ['鱼', '魚'], ['鸟', '鳥'], ['风', '風'], ['电', '電'],
  ['话', '話'], ['语', '語'], ['说', '說'], ['谈', '談'], ['议', '議'], ['认', '認'],
  ['识', '識'], ['见', '見'], ['观', '觀'], ['规', '規'], ['视', '視'], ['贝', '貝'],
  ['财', '財'], ['质', '質'], ['员', '員'], ['单', '單'], ['产', '產'], ['业', '業'],
  ['务', '務'], ['应', '應'], ['继', '繼'], ['续', '續'], ['断', '斷'], ['济', '濟'],
  ['坏', '壞'], ['环', '環'], ['压', '壓'], ['严', '嚴'], ['专', '專'], ['虽', '雖'],
  ['随', '隨'], ['险', '險'], ['难', '難'],
  // 一簡對多繁者只在明確詞組裡修：
  ['尋获', '尋獲'], ['征集', '徵集'],
  // ── 第 1 層：大陸用語（長詞在前）──
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const cn = [
    ...
    ['願受影响之地早日復原', '願受影響之地早日復原'],           // 簡體單字層
    ['搶险人員持續作业', '搶救人員持續作業'],                    // 簡體先正規化，詞層才比得中 搶險→搶救
  ];
