...
export const GATES = [
  ...
  {
    // 🔴 2026-08-21 建。籤詩的九個現代情境是 215 首 × 9 段一段一段寫出來的，
    //    人工逐條看不現實，而首批就混進兩處外文（俄文 негативной、英文「可望轉good」）。
    //    ⚠️ 第一版字元檢查**允許 A-Za-z**（想留給專有名詞），結果漏掉後者——
    //    允許清單開太寬等於沒檢查。現在改成連續外文字母一律擋。
    id: 'check:scenario-text',
    label: '籤詩情境文字',
    needs: 'source',
    tier: 'fast',
    stages: ['pre-push', 'ci-pre-build'],
    changed: ['article'],
    why: '九個現代情境欄位不得混入外文、不得為空、長度須在 10~120 字',
  },
  {
