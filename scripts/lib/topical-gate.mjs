...
export function gateAndFrame(c, { logTag = '[topical]', cadenceNote = 'P1 每 20 分跑一次', srcFallback = '來源' } = {}) {
  ...
  // 🔴 措辭是「那幾個字」而**不是**「那三個字」——2026-08-19 經用戶確認定案，別再改回去。
  //    緣由：合併 orchestrate 與 news-scan 兩份 gateAndFrame 時，orchestrate 那份寫的是
  //    「只准照抄上面那三個字」。但 CWA 正式中文譯名並非一律三字
  //    （docs/topical-blessing.md §3.5 明載「紅霞」＝2 字、「杜蘇芮」＝3 字），
  //    寫死字數在遇到 2 字名時與事實矛盾，LLM 可能因此補字或改寫。
  //    故採不預設字數的寫法。
...
任務(1) 相關性＋正向議題判定，pass 需同時滿足：
  ...
  c. **災害已經實際發生在人身上，不是只發生在地圖上**——**沒有災害就不用祈福**。兩類分開看：
     ・地震、山崩、橋樑坍塌、氣爆、建物火災、建物倒塌這種「發生即是災害」者：事件本身就直接作用在人與房舍上，天然符合本條。
     ・**航空／鐵路／海難／工安／群眾推擠這一族（2026-08-22 新增類型）一律走下面那條嚴格標準，不得歸進上一類**：
       交通與工安事故**經常沒有人受傷**（機尾觸地、衝出跑道、車輛出軌後全員平安、廠房設備損壞），
       事故本身不等於災害。必須有**人員傷亡或失聯**才算，只有機具受損、航班取消、設施關閉一律 block。
       實例：2026-08-15 越南航空 VN34 在慕尼黑機尾觸地並衝出跑道，**無人罹難**——那不該開祈福頁。
