...
//
// folk（神酷）療癒腔：與掃 UI .astro 的 scripts/check-copy-voice.mjs 同源黑名單，只是套用面不同——
// check-copy-voice 掃 src/**/*.astro（產品 chrome 文案），本檔掃 src/**/*.md(x)（典故/籤解等文章正文），
// 兩者互補。folk 的療癒腔（假釋懷、情感詩、「當第一個」勸誘框）不論出現在 UI 或文章都該擋，規則同源。
// 新增禁語時：UI 面補 check-copy-voice.mjs 的 BANNED，文章面補這裡，兩處同步。
const SITE_ERROR_TELLS = [
  ["folk 療癒腔·假釋懷", /放下了|釋懷了/],
  ["folk 療癒腔·情感詩", /(添|多)了一分暖|不是一個人走過|照亮彼此|你的消息會陪/],
  ["folk 療癒腔·當第一個勸誘框", /你(可能|可以|也許|說不定|或許)(會)?是第一個/],
];
