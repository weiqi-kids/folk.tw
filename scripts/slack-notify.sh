#!/usr/bin/env bash
# folk.tw 專屬 Slack 通報小工具（本機 cron / headless 流程用）。
# 與 evidencetoday 完全分離：用 folk.tw 自己的 bot（App「好棋寶寶 Claude 助手」）。
#
# 用法：
...
#   2. 檔案 /root/.config/folk-tw/slack-bot-token（chmod 600，單行 xoxb-...）
# Bot 需 chat:write（有 chat:write.public 則發公開頻道免邀請）。
# 設計：缺 token / 失敗都「不中斷呼叫端」，印警告並回非零，呼叫端應以 `|| true` 包住。
set -uo pipefail

TOKEN_FILE="${FOLK_SLACK_TOKEN_FILE:-/root/.config/folk-tw/slack-bot-token}"
