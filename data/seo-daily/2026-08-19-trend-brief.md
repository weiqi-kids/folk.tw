# 外部搜尋趨勢自動優化 brief｜folk.tw｜2026-08-19
...
- GSC 本站交叉證據：24 條目（2026-08-10..2026-08-16）

## 可進入自動內容優化佇列
### 1. 土地公籤第參拾捌籤（分數 57｜refresh-existing-page）
- 訊號：／上升｜來源：Bing Webmaster。
- 對應頁面：[既有神明頁](https://folk.tw/deities/)。
- 為什麼進佇列：Bing Webmaster 查詢表現上升或首次出現；對應既有頁面「既有神明頁」。
- 安全自動化範圍：src/data/deities.json；只調整閱讀動線、入口或不含法規事實的文案。
- 來源查證：來源查證、日期與站內搜尋意圖；任一項不成立就不自動發布；未找到官方來源就不寫，自動流程直接跳過。
- 來源連結：[Bing Webmaster](https://folk.tw/)。
### 2. 六合彩籤詩網站（分數 57｜refresh-existing-page）
- 訊號：／上升｜來源：Bing Webmaster。
