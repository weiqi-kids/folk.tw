...
>
> 1. **匯入慶典資料** → **2,558 間廟**可拿到真實 `main_festival`（現況只有 21 間）。
>    資料已在 `/root/.config/folk-tw/intake/inbox/religion-festival/`（repo 外，含電話＝個資）。
>    **動手前必讀 [`docs/festival-data-import.md`](docs/festival-data-import.md)** ——
>    裡面有兩個會產生假資料的陷阱（ODS 日期沒有農曆/國曆標記；廟宇 id 是陣列索引不是 MOI 編號），
>    以及三個要先問用戶的取捨規則。這是對 8/31 目標幫助最大的一項。
