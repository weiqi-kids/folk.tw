...
>
> 1. ~~**匯入慶典資料**~~ ✅ **2026-07-31 完成**：**2,498 間廟、4,106 筆**年度慶(祭)典已進
...
> 📊 **台灣端現況要看數字，跑指令、不要讀死值**：`node scripts/intake-status.mjs`
> （`--brief` 只看待處理與抓取失敗）。四段報告＝已在站上發揮作用／還沒處理好／偵察結論／放棄。
> 脈絡與每份資料的整合方式＝**[`docs/taiwan-intake-status.md`](docs/taiwan-intake-status.md)**。
> 🔴 **授權判準是「這份資料在不在 data.gov.tw 上」，不是「從哪個網址抓的」**——
> `religion.moi.gov.tw` 同時供應 OGDL 開放資料（temple.xml 8203／Festival.xml 8209…）
> 與受版權宣告保護的網站內容（沿革／建築特色／參拜流程／`Knowledge/*`），後者未獲書面同意不得轉載。
