...
| **`/festivals/local/` 上線後看曝光** | 2026-08-06 上線（65 項地方宗教慶典＋回灌節日頁與廟宇頁）。裡面是鹽水蜂炮、大甲媽祖遶境進香、頭城搶孤這類頭部詞，但**站上完全沒有它們的歷史曝光**，所以現在無從判斷值不值得再擴（縣市頁）。看兩週 GSC 再決定，別先擴。<br>⚠️ **2026-08-18 提前查證（8/20 到期前兩天）：已轉為 `Submitted and indexed`**（`lastCrawlTime` 2026-08-16，非 8/12 那次的 `Discovered - currently not indexed`）——但**收錄之後、曝光依舊是 0**（8/06–8/17 逐日曝光全 0，查詢詞 0 筆），referringUrls 也是 0，代表 Google 連結圖尚未記到內站連結。作對照：同批（搶孤 `/festivals/qianggu/`）轉入索引後 8/11 起立刻有曝光且逐日成長（8/16 已 226 曝光/11 點擊、平均排名 4.8）。🔴 **`/festivals/local/` 已排除「未收錄」這個解釋，8/20 若仍是 0 曝光，代表卡點在別處**（多半是站內連結權重不夠或需求本身弱於搶孤這類已有歷史曝光的頭部詞）——屆時判斷方向前先重跑一次 `inspectUrl` 確認 referringUrls 是否已非 0，若仍 0 才是真的內鏈問題。查法：對該頁重跑 URL Inspection＋GSC page 篩選，不要讀 `index-audit.json` | **8/20** |
