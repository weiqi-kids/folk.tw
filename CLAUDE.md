...
- [x] **nav 改版：13 個扁平項目 → 7 組（2026-07-31，用戶裁示）**：行動版佔流量 **70%**
      （GA4 實測 2,369/3,389），一長串扁平連結在小螢幕是一面牆。四點命名修正皆用戶決定：
      ① 首頁錨文字**不可寫「運勢」**——首頁實際區塊是熱門神明／近期節日／近期神明聖誕／今日農民曆，
      **站上沒有運勢內容**；nav 是全站 12,000 頁指向首頁的錨文字，用「運勢」會把主題認知帶往我們給不出的東西。
      ② 「籤詩」正名「解籤」（與「我想…」的「求籤」區分：求籤＝抽一支，解籤＝查第 X 籤；
      GSC 實測籤詩頁查詢多為「第X籤」）。③ 擇日全收進「我想…」，農民曆退回工具層（避免兩入口通往同一批資料）。
      ④ 「近期節日」→「節日」（錨文字用搜尋詞，不用介面修飾詞）。
      - **分組必須是靜態 `<details>`／`<summary>`**，不可由 JS 注入——爬蟲未必看得到，
        且會踩 Astro scoped style 套不到 JS 注入 DOM 的坑（記憶 [[astro-scoped-style-innerhtml-pitfall]]）。
      - 順帶接回三個**原本完全不在 nav 的孤兒樞紐**：`/compare/`、`/jiaobei/`、`/vocabulary/`。
...
- [x] **`/good-days/` 擇日專區（2026-07-31）**：slug＝**永久承諾**，用戶指定現代英文
      （`wedding`／`moving`／`groundbreaking`／`burial`／`worship`；後續 `haircut`／`business-opening`／
      `bed-placement`／`travel`）。宜日一律由 `src/lib/almanac/select.ts` 的 `verifiedYiDays` 取得（M3 唯一入口）。
      - 一頁可對多個用事（`worship`＝祭祀＋祈福），**分組列出不合併**成一個模糊的「宜拜拜」。
      - 🔴 **20 個用事只有 10 個有料**：求嗣／開光／出行／納采／上樑／**安床**／**開市**／移徙／破土／入殮
        **恆空**（投票表只有忌票、沒有宜票，結構上永遠推不出宜日）。安床與開市是擇日類搜尋量最高的兩個詞。
        補完用的是同一份卷十一原文、同一套工序，**優先度高於新增用事**。
