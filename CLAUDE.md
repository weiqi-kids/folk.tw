...
| **宮廟開發名單的前提正在鬆動** | folk-outreach（每日五間，台北 04:30 推 Slack）挑的是「有曝光但頁面還缺沿革／官網」的廟——**內政部沿革匯入後，被「已經有內容」刷掉的數量暴增**：log 逐日 `已有內容N` 為 `0 0 0 1 1 1 1 102 294 391`（末筆 2026-08-08）。⛔ 還沒到要停的程度（當日候選池仍數百間），但每批 MOI 到貨都會再啃一塊。<br>👉 接手時先跑 `grep -oE '候選池 [0-9]+' /root/folk.tw-outreach/outreach.log | tail -5` 看還剩多少。見底時的選項與**「別默默降門檻硬湊五間」**的理由寫在 `/root/folk.tw-outreach/README.md`（該目錄在 repo 外，⚠️ 不受 `check:doc-numbers` 管，數字漂移沒有東西會擋）。<br>⚠️ **2026-08-17～08-20 這個 job 死了四天沒人發現**：目錄在 8/17 被改名為 `/root/folk.tw-outreach`（同時建成 repo `weiqi-kids/folk.tw-outreach`），`/etc/cron.d/folk-outreach` 的路徑沒跟著改，node 直接找不到檔案，連腳本內建的「失敗也發 Slack」都沒機會跑。2026-08-20 已修正 cron 並補跑當日一輪。🔴 **repo 外的東西改名／搬家，同一回合要 grep `/etc/cron.d/` 有沒有人指著它。** | 無期限 |
| **鬼門開與地藏頁仍與中元節頁重複** | ⚠️ **2026-08-12 更正：這兩頁早就被收錄了**（URL Inspection 實測皆 `Submitted and indexed`、8/07 爬取），所以問題**不是收錄**、是內容差異化。同日重量測線上 `<main>` 重疊：鬼門開 44.3%、地藏 49.3%（幾乎沒動），而同批的雞籠 22.4%／義民 19.2%／清明 20.0% 都拉開了。<br>2026-08-09 把 `pudu` 的完整流程收斂到主場頁、並替搶孤／雞籠／義民／清明補了獨有內容後，**這兩頁是唯一沒改善的**（當日實測與 `/festivals/zhongyuan/` 的 `<main>` 內文重疊：鬼門開 45.4%、地藏 49.9%；樣板底噪約 10%）。⛔ 卡點：**兩者都沒有自己的文化資產個案可引**——「鬼門開」與「地藏王菩薩聖誕」不是文資登錄項目，所以 2026-08-09 那條文化部授權（逐字引用＋掛個案網址）對它們沒有用。👉 下一步得另找權威源。✅ **2026-08-19 解鎖**：國史館臺灣文獻館（`th.gov.tw`，有〈七月半之緣起〉一文）經站主確認已授權，條件＝標示資料來源連結，該網域已進 `check-source-refs.mjs` 的 `VERBATIM_ALLOW`，**可以逐字引用了**。所以現在卡的是「還沒有人去寫」，不是「沒有源」。<br>🔴 **不要用「再補內鏈」解**：2026-08-03 對同一批頁做過、無效（`jilong-zhongyuan` 曾有 191 頁連過去仍是 unknown to Google）。<br>重量測：`node -e "..."` 逐頁比 `<main>` 內文 8 字片段重疊率，作法見 [`docs/decisions/festivals-and-intent.md`](docs/decisions/festivals-and-intent.md) 的「節日頁自我重複」條目 | 無期限 |
...
🅤 **商業模式的立場（2026-08-12 用戶更正、2026-08-20 再確認）**：**不接受業配文，但不排除廣告投放**；
站上本來就規劃 NFC 抽籤這類產品。`/about/` 原本寫的「本站不販售亦不用於廣告投放」是**建站時的誤解**，
已移除——**不要再以任何形式寫回去**（那是替站主對外做他沒同意的承諾）。
理由與界線見 `src/pages/about.astro` 檔頭。
⚠️ 這條**不鬆動**祈福頁的「不變現」紅線（災難頁專屬，見 [`docs/topical-blessing.md`](docs/topical-blessing.md) §0.1）。

🔴 **由上一條直接推出的授權紅線（2026-08-20 站主明示「有商業行為」）**：
**任何 NC（姓名標示─非商業性）授權的素材一律不可用**，不分來源、不分平台，也不要「先收著再說」。
向館方／機關申請授權時**用途欄照實填有商業行為**，不可為了省申請費填非商業。
⚠️ 這條**不影響** PDM／CC0／CC BY／CC BY-SA／OGDL——那幾種都不限制商業使用。
判準與逐件實例見 [`docs/qian-systems-sources.md`](docs/qian-systems-sources.md)。
