...
| **廟宇照片沒有可用的來源** | ✅ 神明代表圖那半邊**已上線**（2026-08-07，`religion-photos` job）。<br>⛔ 廟宇照片那半邊**是死的**：`GetUploadFile` 的 pic 附件，其 JSON 自報的 `URL` 是 `/ReligionSys/FileStore/<GUID>.<EXT>`，該路徑一族**來源本身 404**（台灣端兩次複驗）；那網址不是我們拼的，**沒有別的網址可以產**，已在 `gen-intake-urls-photos.mjs` 濾掉整族。要廟宇照片得另找來源，不是修抓取。<br>🔴 **署名查不到就不採用那張圖**——寧可沒圖，也不能掛一張沒署名的別人的作品；圖說在條目頁是**在 `<img>` 之後**（曾掃錯方向導致署名全空、圖被安靜略過）。 | 另找來源才有 |
