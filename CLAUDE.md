...
| **照片匯入器還沒寫** | ✅ **job 已開**（2026-08-07，manifest v14 `religion-photos`）——原本卡的「候選量太少」在新增 18 尊有內政部條目的神明節點後解除。⛔ 現在卡的是**下一段**：`scripts/import-photos.mjs` 尚未存在，照片抓回來也進不了站。🔴 **每一張都必須在頁面上印攝影者姓名**（內政部圖說寫了攝影者，屬著作人格權的姓名表示，不是可選的），匯入器與渲染層都要做，並補 gate。 | 等台灣端送檔 |
