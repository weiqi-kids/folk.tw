...
- **SERP 表現：title／description／分享卡**
  - 廟宇頁 meta description 去樣板（2026-07-30 同批，影響 7,89
  - 廟宇頁 title 加「主祀○○」（2026-07-31，7,801/7,891 頁生效）：
  - 廟宇專屬分享卡（2026-07-30，用戶指示）：外撥把廟宇連結傳給廟方時，原本 12,01
- **頁面內容與內鏈**
  - 廟宇鄉鎮二級瀏覽（2026-06-24）：`temple-region` 加 `temple
  - 廟宇頁在地脈絡（2026-07-28）：GSC 抽驗推估約 810 頁未索引，最單薄那批只有
  - 觀光署景點簡介／開放時間匯入（2026-08-05）＋🔑 授權界線（改動前必讀）：
  - 年度慶(祭)典匯入（2026-07-31，2,498 間廟／4,106 筆）：內政部全國宗教
- **資料維護：座標與外撥**
  - 缺座標廟宇地理編碼回填（不需 MOI XML）：`node scripts/geocode-
  - 廟宇座標回填（可重複、安全閘內建）：`pnpm data:temple-coords <te
  - 宮廟開發名單「每日五間」（2026-07-30 建，用戶指示）：每天台北 04:30 推 S
...
- **廟宇專屬分享卡（2026-07-30，用戶指示）**：外撥把廟宇連結傳給廟方時，原本 **12,018 頁共用同一張**
  ...
  - ✅ **2026-07-31 已取得真正的廟宇活動**：卡面第三行現在依序取 `main_festival`（21 間查證敘述句）
    ...
    見**本檔上方**「年度慶(祭)典匯入」條目（2026-08-06 重整後該條移到前面）。以下為當時的探源紀錄，**別重做一遍**：
