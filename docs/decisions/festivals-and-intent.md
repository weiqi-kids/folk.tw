...
- [x] **Article／Discover 資料訊號：`datePublished`／`dateModified`／`author`／`image`（2026-08-12）**
      ...
      **不可改成 build 時間**——全站每日 cron 重建，掛 build 時間＝天天謊報全站更新，
      ...
      🔴 **`author` 用既有 `ORG`（Organization），不掛個人姓名。** 站上條目是彙整＋逐條掛源，
...
- [x] ✅ **2026-08-19（同日稍晚）更正：`taiwangods.moi.gov.tw` 已取得內政部確認授權。**
      站主回報內政部已確認，條件比照 2026-08-06 那次＝標示資料來源連結。
      `check:source-refs` 已把它移出未授權名單、加入逐字白名單，grandfather 上限一併移除
      （那條規則的前提是「未授權來源不得再擴散」，前提消失就不該留著擋正常使用）。
      `/about/` 的敘述同步更正——授權確認後那句「尚未取得同意」就是線上頁面在說錯話。
      ⚠️ **下面這段保留為當時的判斷過程，不是現況**；它促成的那些改動（修掉掛錯的引用、
      把近似逐字段落改掛 nchdb）**不回退**，因為那些本來就是掛錯頁或等價來源，與授權無關。

- [x] ~~**`taiwangods.moi.gov.tw` 的授權是站上目前最大的來源風險**~~（已由上一條解除）：版權宣告逐字寫
...
- [x] **國史館臺灣文獻館（`th.gov.tw`）未查到明示授權**：主站著作權聲明抓不到（SPA＋常見路徑
      ...
      → **可摘述、不可逐字大段引用**。
      ⚠️ **2026-08-19 更正（本條的結論已不適用）**：站主確認此站亦已授權，條件比照前兩次
      ＝標示資料來源連結。`th.gov.tw` 已進 `VERBATIM_ALLOW`，`UNLICENSED`／`BASELINE` 清空。
      上面那段「查不到聲明」的實測結果仍然為真——**它只是查不到，不等於沒有**，
      這正是「所屬／授權關係只能問不能推」的又一次應用。
...
## 2026-08-20 · 民俗活動「舉辦週期」拆欄位（起因：線上輸出病句）

**事故**：curl 實證，`/events/` 底下有三頁在輸出壞掉的句子——
`wanhegong_laoerma_xitun`「…舉行，三年一科**年一科**。」、
`nanguanxian_wangjiao`「…非同步**年一科**」、
`anding_zhenhugong_wangchuan`「…（首科1967年）**年一科**」。

**成因**：`events.json` 的 `ke_rule` **一個欄位同時裝著兩種東西**。
`content.config.ts` 原本的註解寫「三年一科：丑辰未戌」，意思是它只該放地支；
但有值的 10 筆裡只有 2 筆是純地支，其餘 8 筆塞的是自由文字，
而 `z.string()` 攔不住這種語意違反。頁面樣板直接串 `${ke_rule}年一科`，
遇到自由文字就爆。

**處置**（不是修那三頁的字，是讓它結構上不可能再發生）：
- 拆成三個各自單一語意的欄位：`ke_branches`（只放地支，zod enum 擋）／
  `ke_period_text`（週期怎麼講）／`ke_note`（其他補充，不參與造句）。
- 造句收進 `src/lib/event-cycle.ts`，只吃結構化欄位。
- 🔴 **不由地支數量推算年數**：馬鳴山逢寅、午、戌年，算術會得「四年一科」，
  但通行稱法與該筆自己的 `date_note` 都是「五年一科」。推算等於杜撰。
  來源怎麼說就怎麼寫，沒說就留 `null`。

**過程中挖出的第二、三層同型問題**：
- `date_note` 也裝著兩種東西——短片語（「九月」）與整段散文（蕭壠香那筆是三個句子）。
  把「約於 X 舉行」套在散文上會生出「…不是固定的農曆日。舉行。」。加 `noteIsProse()` 分流。
- **重複**：`date_note` 本來就把週期講完了，舊樣板還無條件再接一次
  （安定真護宮那筆週期被講三次）。改成只補 `date_note` 沒講到的部分。
- `events/index.astro` 有**第二份** `CYCLE` 對照表且用詞與詳情頁不同
  （`每年` vs `年度`、`不定期` vs `不定`）——同一個事實在兩頁講不同的話。
  兩份都收進模組後，措辭差異變成兩個具名函式（`cycleLabel` 給詳情頁可長、
  `cycleShort` 給卡片長度可預期），而不是兩份會各自漂移的表。

**gate**：`test:event-cycle`（source／fast，pre-push＋ci-pre-build）。
用 **golden table 逐字釘住答句**而不是幾個 assert——因為判準是對人寫的散文做正規式判定，
正規式漂不漂亮沒有意義，**產出的句子被人看過**才有意義。改了模組而句子變了就會紅，
逼人重看一遍。另含「`ke_rule` 不得復活」與全量掃 7 種病句型態。

**順帶建立的跨資料集不變量**：`local-celebration-cases.json` 的 `verdict` 與
`events.json` 的 `cycle` 透過 `celebration-occurrences.json` 的 `page_ref` 連著，
但從來沒有東西比對過它們。加上比對後照出一筆——見下條。

### ⚠️ 未決：學甲香是三年一科還是四年一科

`/festivals/local/` 說「數年一科」、`/events/xuejia_shangbaijiao/` 說「每年一次」，
乍看矛盾，實查是**範圍差異**：`lc_61「學甲香」`指刈香本身，而該 event 同時涵蓋
每年的上白礁謁祖與數年一次的刈香，所以 `cycle: annual` 對前半而言正確。
這筆已列入 `SCOPE_EXEMPT` 並寫明理由。

**但裡面藏著一個真的事實爭議**：`events.json` 的 `date_note` 寫「刈香**三年**一科」（無掛源），
而 `lc_61` 的 `basis` 引 nchdb `20080627000002` **逐字**「亦每隔**4年**舉行一次刈香」，
且該筆列的維基年份（1988／1992／1996／2000／2004／2008／2012／2016）也是四年間隔。
三年與四年只能有一個對。**這是對文化資產的事實主張，要站主裁示後才改，並補上來源**——
在那之前，未決狀態存放在 `src/lib/event-cycle.test.ts` 的 `SCOPE_EXEMPT.lc_61` 註解裡。
