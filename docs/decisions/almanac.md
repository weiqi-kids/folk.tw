...
- [x] **擇日頁「日子挑好之後」動線（2026-08-02，用戶裁示）＋🔴 決定不做店家名錄**：
      用戶問「能不能接附近的店面資訊」。**站上不存任何店家資料**，只在真的有服務可約的擇日頁
      放一個地圖搜尋深連結（由使用者裝置定位），或連站內既有資料。
      - 目前只有 3 個有　⚠️ **2026-08-06 實查為 5 個**（worship／haircut／moving／wedding／engagement）：`haircut`→地圖搜「理髮」、`moving`→地圖搜「搬家公司」、
        `worship`→站內 `/temples/`（我們已有 7,891 間廟且有座標）。
        資料在 `good-days.json` 的 `next_step`，理由寫在同檔 `_next_step`。
      - ⚠️ **2026-08-06 更正**：下面這句「結婚／訂婚…刻意不放」**已被用戶推翻，照它做會刪掉用戶要的動線**。
        `good-days.json` 實際有 **5 筆** `next_step`（worship／haircut／moving／**wedding**／**engagement**），
        該檔 `_next_step` 註解明寫「2026-08-02 用戶指定**結婚與訂婚都接新娘秘書**——訂婚宴同樣要造型」。
        仍不放的是：安床／開市／出行／上樑／動土（無對應單一服務）與**安葬**（禮儀公司連擇日一起處理）。
      - 🔴 **仍刻意不放的用事**：安床／開市／出行／上樑／動土 沒有對應得上的單一服務，
      ...
      - 🔴 **不做店家名錄的三個實查理由**（別再重開此案，除非數據反轉）：
        ...
        ③ **這是廟宇頁那場仗的重演**：「附近的○○」是純在地意圖，SERP 由 Google 地圖包回答。
           站上實測廟宇頁排名 1–10 的 CTR 只有產業基準的 **0.17～0.42 倍**（11 名後才回到 0.99 倍）。
           拿 2.5% 涵蓋率的資料進同一個戰場，是拿更差的牌打同一手。

## 宜忌條目與判定維度

- [x] **嫁娶宜忌補 11 條＋新增日相標記維度（2026-07-31，commit `39353ab`）**：
      實測 2026-08-17 建除為「平」、協紀卷十一嫁娶條明列忌平日，站上卻列為「宜嫁娶」。
      **根因不是漏填規則，是投票表少了一個維度**——卷十一清單裡的「平日／收日／閉日／亥日／丁日」
      ...
      - Gate：`check:rendered` 不變量 6（原誤編為 5）——把擇日頁列出的每個宜日翻到該日農民曆頁比對建除／日干／日支。
