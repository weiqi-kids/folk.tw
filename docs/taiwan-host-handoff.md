...
| 慶典資料匯入站台 | ✅ **2,500 間廟／4,108 筆已進 `temples.json` 的 `festivals[]`**（見 `docs/festival-data-import.md`） | 2026-07-31 |
...
### 🔑 授權盤點（2026-08-05，決定哪些資料可以放上公開站台）

**判準是「這份資料在不在 data.gov.tw 上、有沒有開放授權」，不是「從哪個網址抓的」。**
同一台 `religion.moi.gov.tw` 同時提供兩種東西，授權完全不同：

| 資料 | 來源性質 | 授權 | 可否發佈 |
|---|---|---|---|
| 寺廟基本資料 `Report/temple.xml` | **data.gov.tw 8203** | **OGDL 1.0** | ✅ 可（站上 7,891 間的基礎） |
| **慶(祭)典 `Report/Festival.xml`** | **data.gov.tw 8209** | **OGDL 1.0** | ✅ 可 |
| 景點描述（363 間對得上） | data.gov.tw 7777（觀光署） | OGDL 1.0 | ✅ 可 |
| 慶典查詢**網頁／ODS 匯出** | 網站 UI，非開放資料集 | 站台版權宣告 | ⚠️ 見下 |
| **歷史沿革／建築特色／參拜流程全文**（`GetUploadFile` 的 `Comment`） | 網站內容，**任何開放資料集都沒有** | 站台版權宣告 | ❌ **不匯入** |

`religion.moi.gov.tw` 版權宣告（`Home/ContentDetail?cid=Copyright`）關鍵兩條：
> 二、**在限於個人及非商業目的的情況下**…自由瀏覽及使用本網站，或下載本網站上**明示提供下載**之相關資料，惟公開時需註明出處。
> 三、**任何商業機構或團體**，非經內政部以及各版面著作人書面同意，**不得以任何形式轉載、重製、散布、公開播送、出版或發行本網站內容**。

- **沿革全文＝語文著作**（有創作性表達），把 2,772 段放上公開站台就是第三條講的「轉載、重製、散布」。
  **在取得書面同意前不做**（第五條有洽詢管道，那是用戶的決定，不是我能替他認定的）。
- **慶典的名稱／日期／廟名＝事實資料**，且**同一份資料本來就以 8209 開放授權發佈**，
  因此站上已匯入的 4,108 筆沒有授權問題。
  ⚠️ 但取得管道應該改走 **`Report/Festival.xml`（8209）**，而不是爬 7.5 MB 的查詢網頁——
  同樣的事實、乾淨的授權、機器可讀。**唯一的顧慮是 XML 的欄位表沒有「曆別」**
  （慶祭典名稱／宗教團體／類別／日期／區別／地址），而曆別是 `docs/festival-data-import.md`
  明列「HTML 是唯一可用主來源」的理由。**先讓台灣端抓一份實物核對再決定**，不要憑欄位表就切換。
| CRGIS 重測（第 9 次） | ⚠️ 仍 HTTP 000（curl exit 28）。維持 `_alert: false`＋每輪重試，不必再單獨回報 | 2026-08-05 |
| `temple-xml` 三檔去向（台灣端 8/03 提問） | ✅ **有收到、已正常匯入**。境外上位檔 6,270,955 bytes、sha256 `8f283b1b…306e` 與台灣端本地檔一致；舊版在 `archive/temple-xml/2026-08-01/`。inbox 看似空是設計如此（ingest 驗完 sha 即原子 rename 上位） | 2026-08-05 |

**境外端已算出、台灣端算不到的那一半**（對映到站上實際收錄的 7,891 間）：
沿革 **2,772 間**（站上現有 22 間＝126 倍）、建築特色 2,399 間、參拜流程 **556 間**。
這是這批偵察真正的價值所在——比原本要找的「服務項目」更有用。
...
2. **慶典 job：請出草案，會採用。但你的報告有兩處要更正**（境外端實測）：
   - **ODS 不能當主來源**。你建議「ODS 拿乾淨資料、HTML 補電話與宗教別」——但 ODS 的日期欄
     **完全沒有農曆／國曆標記**（實測前 200 筆全無），而全量 6,644 筆中農曆佔 6,365。
     照那個做法會把農曆當國曆，媽祖聖誕會從 4/29 變成 3/23。**HTML 是唯一可用的主來源。**
   - **98.3%／6,488 間是對 `temple.xml` 的 12,419 筆算的**，不是站上實際收錄的 7,891 間。
     對站台而言正確數字是 **2,500 間（31.7%）**——**2026-07-31 已實際匯入完成**（4,108 筆）。
     差額來自：1,796 筆慶典的廟我們沒收錄、290 筆同名且行政區無法消歧（刻意捨棄，避免張冠李戴）、
     以及 439 列資料清洗（名稱無漢字如 `.`／`33333`、帶絕對年份的一次性日期、`07/00` 這類不存在的日子）。
     （偵察階段曾推估 2,558，那是**清洗前**的數字。）
     ⚠️ 同名消歧另有一招有效：**「廟名＋電話」對 `temple.xml` 再用完整地址對回站台**，
     救回 99 筆。電話只在記憶體中比對、不寫進 repo（個資，本 repo 為 public）。
