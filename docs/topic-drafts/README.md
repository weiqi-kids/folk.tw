...
這份是實際寫作交付記錄，不是候選題目清單。每個月的內容都已寫入對應 markdown 證據包，包含可放進正文的 lead／段落、官方來源、FAQ、canonical 分流與重複風險。這些檔案仍在 `review-gate`，不會自動變成公開頁面。
...
主題草稿 gate 通過：6 檔，0 errors，0 warnings
```

gate 會檢查：每檔至少 3 個可追溯 URL（其中至少 2 個官方／公立來源）、搜尋意圖、FAQ、合併／重複風險、年度來源狀態、圖片／OG／授權檢核、模板化套語，以及跨檔 8-gram 重複率；路由／編號內容單元也要各自有最低正文長度與鄰近來源標記，不能用整份長文件掩蓋單一薄段落。
