...
- ⚠️ **殘留缺口（P2→P1 方向未關）**：若 P2 先為某颱風開了頁，P1 之後才從 GDACS 撈到同一個颱風，
  **擋不住**——GDACS 只給英文名與 `place=China` 這類粗地名，我方欄位裡沒有中文名可比對，名字規則無從發動。
  P1 的去重仍是它自己那份 `sameEvent`（地震 250km/1 天、其餘 10km/3 天），**未改**（換成 lib 版會讓地震
  跨觀測網去重從 250km 退化成 10km）。實務上 P1 每 20 分跑、P2 每 8 小時跑，P1 先發現的機率高得多，
  故此方向風險較低；真發生了走 §3.6 併頁。
