...
`findDuplicate(list, cand)` 四道規則，任一命中即不開頁：
...
- **簡繁轉換殘字收在同一張表**（`征集→徵集`、`获→獲`）：性質相鄰、同樣一對一可修；
  只收在正體語境中**不可能正確**的字形。
...
- 改事件/更新文案的**產生方式**（prompt）→ 必須確認**三道硬 gate（§3）仍有效**；prompt 只是第一層。
