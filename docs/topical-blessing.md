...
- **颱風標題要有名字（2026-07-26 修）**：同一座橋也用在**產標題**那條路上，不只去重。原本 `gateAndFrame()`
  的 prompt 只餵類型/地點/日期/嚴重度/摘要，名字只以英文代號夾在 GDACS 摘要裡（`… NOUL-26 was active …`），
  而 prompt 又（正確地）禁止自創譯名 → LLM 只能略去不寫，標題長成沒有主角的「**為中國熱帶氣旋平安祈福**」
  （紅霞頁 7/23～7/26 就是這樣上線的）。修法：偵測時除了 `cycloneName` 另存 **`cycloneNameZh`**（查 CWA 對照表，
  查無＝大西洋颶風那類則 `null`），`gateAndFrame()` 有中文名時 **(a)** 標籤由「熱帶氣旋」改台灣慣稱「**颱風**」、
  **(b)** prompt 多一行「颱風名稱：紅霞」＋一條硬性要求「標題必須寫出名字、只准照抄」。查無中文名時完全維持舊行為
  ——**中文名只能查表、絕不讓 LLM 音譯**（紅線：絕不杜撰）。
