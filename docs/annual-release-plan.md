...
建議 manifest 欄位（本輪只定義契約，沒有新增 JSON）：
...
6. **發布後：只提交已上線 URL。** sitemap 自動包含已發布頁；IndexNow／GSC URL Inspection 僅處理本批已驗證 URL，不預先送未發布清單。用 GA4／GSC 觀察 7—14 天，再決定下一批是否需要調整或合併。
