...
🔎 **來源盲點另跑這支**——它把內政部「地方宗教慶典」逐項和現有 `/festivals/`、`/events/` 對帳，
並區分「已有專頁」「只有同日總頁」「沒有對應頁」。它只產候選，不把來源清單當需求、不自動開頁；
近期候選仍須逐筆查 GSC query×page，再套 §3 三條件：

```bash
pnpm growth:source-candidates
pnpm growth:source-candidates -- --days 60
pnpm growth:source-candidates -- --as-of 2026-08-22 --json
```
