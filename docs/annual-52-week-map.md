...
| 19 | 內門宋江陣與地方藝陣 | `/events/neimen/` | `published-refresh` | 以文化局／主辦方資料補年度檔期，保留地方事件脈絡。 |
...
| 25 | 學甲上白礁暨刈香 | `/events/xuejia_shangbaijiao/` | `published-refresh` | 以主辦方與地方文化資料更新年度程序。 |
...
| 30 | 東港迎王、西港香 | `/events/donggang/`、`/events/xigang/` | `published-watch` | 只在當年度確定舉行後更新；不同地方王醮不可合併成通用日期。 |
...
| 32 | 玄天上帝香期與地方進香 | `/events/mingjian_shoutiangong_xuantian/` | `published-refresh` | 更新受天宮／地方公告；神明職司回到 `/deities/`。 |
| 33 | 求平安、收驚與補運 | `/scenarios/qiu-pingan/`、`/practices/shoujing/`、`/practices/buyun/` | `published-merge` | 以情境頁回答「我該從哪裡開始」，習俗頁回答做法；不新增症狀型薄頁。 |
| 34 | 求姻緣、求子與對應神明 | `/scenarios/qiu-yinyuan/`、`/scenarios/qiu-zi/`、`/compare/` | `published-merge` | 監測查詢與點擊，優先補既有頁的比較段落，不批量建神明生日頁。 |
| 35 | 搬家入厝、安神位 | `/scenarios/banjia-ruzhai/`、`/practices/ruocuo/`、`/practices/anshenwei/` | `published-merge` | 清楚區分情境建議與儀式步驟；廟方服務細節逐廟掛源。 |
| 36 | 收驚、補運與祭解 | `/practices/shoujing/`、`/practices/buyun/` | `published-watch` | 追蹤 GA4 互動與來源，避免用療癒套語擴寫；不足時維持短而有源。 |
...
| 43 | 神明生日與多說並列 | `/deities/`、神明頁 birthday | `published-merge` | 以神明圖譜承接生日查詢；有諸說並列來源，不批量生成節日薄頁。 |
| 44 | 神明比較：媽祖／觀音／關帝等 | `/compare/` | `published-watch` | 依 GSC／GA4 代表頁觀察再擴寫；不為每組比較自動產文。 |
| 45 | 情境入口：考試、開店、健康、訴訟 | `/scenarios/` | `published-watch` | 八個既有情境輪替觀察；只對出現新意圖且有來源者提出新頁。 |
| 46 | 農民曆今日宜忌與節日對映 | `/almanac/`、`/almanac/month/` | `published-watch` | 維持月份樞紐與每日工具；不把大量日期頁送進 sitemap／GSC。 |
| 47 | 宮廟地方搜尋與附近工具 | `/temples/`、`/temples/region/`、`/temples/nearby/` | `published-watch` | 以 GA4／GSC cohort 找真正需求；不因資料量批量擴寫樣板廟頁。 |
| 48 | 地方宗教慶典清單 | `/festivals/local/`、`/events/` | `published-refresh` | 更新政府登錄資料與曆別；留空的廟宇對映不可為了湊數強連。 |
| 49 | 時事祈福事件與後續發展 | `/qiugian/`、`/qiugian/blessing/` | `published-watch` | 同一事件合併 updates；只有新事件／新地點才開頁，並驗 timeline 是否渲染。 |
| 50 | 籤詩、典故與每週選讀 | `/poems/`、`/allusions/`、`/systems/` | `published-watch` | 以既有籤詩與典故做每週入口，不一次批量製造新解籤頁。 |
| 51 | 儀式 FAQ、詞彙與跨頁導覽 | `/practices/`、`/vocabulary/`、`/compare/` | `published-merge` | 把查詢導向正確 canonical；以重複率 gate 阻止換詞副本。 |
| 52 | GA4／GSC 回顧與下一輪選題 | `/`、`/festivals/`、內部報表 | `published-watch` | 檢查 7–14 日收錄、CTR、活躍使用者與互動，再決定下輪 2–4 個釋出題目；不新增 URL。 |

## 盤點結論

- 已有內容不是「每週缺一頁」：主要節慶、地方活動、習俗、情境、比較與神明圖譜都已有 canonical。52 週的工作核心是年度刷新、事件更新、內鏈分流與數據判讀。
- 明確缺口集中在重陽、冬至、端午、一般元宵、下元等；它們目前應停在 `source_required` 或 `published-merge`，不是立刻批量開頁。
- 品質 gate 目前把存量不足列為 WARN；新／年度 `scheduled` 頁則必須有清楚 intent、至少 3 個不重複且逐句掛源 facts、可追溯來源、圖片與跨頁重複審查。
- 每週槽不等於每週發布。實際發布以月度小批為單位，單批預設 2–4 個真正不同意圖的 canonical；沒有證據的一週可以只更新既有頁或留白。
