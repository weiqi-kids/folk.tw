...
export const interpretationsSchema = z.object({
  ...
  // ── 現代處境的解讀（2026-08-21 起）──────────────────────────────
  // 🔴 為什麼要有這一組：籤詩頁的九個分項用的是**傳統詞**（功名／姻緣／求財…），
  //    而使用者搜的是**現代的詞**——GSC 實測有人在搜「纔發君心天已知換工作」
  //    「言語雖多不可從求職」「耕耘只可在鄉邦復合」，我們排在第 15–21 名，
  //    因為頁面上根本沒有「換工作」「復合」這些字。兩組詞的交集是 0。
  //    需求是實測存在的，不是推測：考試升學 96 曝光、買房搬家 89、復合 52、換工作 21
  //    （2026-06-01~08-18；而且那是我們排第 2–3 頁時的數字）。
  // ⚠️ 這一組是既有九項的**具體化**，不是新事實：依該籤四句、卦頭典故與既有分項解而寫。
  //    寫作界線同既有分項——文化呈現、不對個人預言吉凶、不寫成「你應該怎麼做」。
  //    🔴 頁面上**不加任何免責或說明性警語**（站主 2026-08-21 明示：那些都在 /about/）。
  換工作: z.string().optional(),
  求職面試: z.string().optional(),
  創業開店: z.string().optional(),
  投資理財: z.string().optional(),
  考試升學: z.string().optional(),
  告白追求: z.string().optional(),
  復合挽回: z.string().optional(),
  分手離婚: z.string().optional(),
  買房搬家: z.string().optional(),
  draft: z.boolean().default(false),
});
