...
/**
 * 「資料集級整包宣稱」——**不渲染到頁面上**，但保留在資料層。
 *
 * 🔴 2026-08-21 站主在 `/temples/moi_8503_臺中市元保宮/` 抓到：頁尾「來源：」的第一段是
 *    「內政部全國宗教資訊網（data.gov.tw/dataset/8203）」，**沒有 url、點不開**，
 *    而且**一模一樣地出現在 10,684 個廟宇頁**。原話：「出處等資料不是在頁面中，
 *    這些是要在 /about/ 頁面啊！這會十分影響使用者體驗！」
 *
 * 這正是 2026-08-07 已經裁示過的那條界線的漏網（當時掃的是 .astro 裡寫死的句子，
 * 沒想到同一句話是從**資料層**每筆餵進來的）：
 *   ❌ 擋：「本頁／本站的資料來自 X」這種**資料集級**宣稱——`/about/` §版權與授權
 *          已經逐條在講（第 184 行「內政部寺廟開放資料：依政府資料開放授權條款使用、標示來源」），
 *          每頁再重複一次就是把頁面變成免責聲明牆。而且它**沒有連結**，
 *          對「標示資料來源連結」這個授權條件一點貢獻都沒有。
 *   ✅ 留：**逐筆事實掛源**（指向該筆自己的原始網址）——那是授權義務本身，
 *          `/about/` 明寫「本站的作法是逐筆掛回該筆自己的原始網址」。
 *
 * 🔴 判準是**白名單**不是「有沒有 url」：《台灣的神明》、干寶《搜神記》這類書目同樣沒有 url，
 *    但它們是**某一筆事實的引用出處**，不是整包資料集的宣稱，砍掉就變成無源陳述（違反總紅線 1）。
 *    要新增項目就往這個清單加，並且加的時候問一次：「/about/ 講過了嗎？」
 *
 * ⚠️ 只擋渲染、**不動資料**：`sources` 欄位仍完整保留（授權紀錄要能被機器複驗），
 *    匯入器也不必改（改了它重跑一次就長回來）。
 *
 * 硬 gate＝`pnpm check:copy-voice` 的 `SOURCE_CLAIM`（2026-08-21 同批補了三條：
 * 「內政部…開放資料」不論括號、「某某資料：某部會」句型、`data.gov.tw/dataset/8203|7777` 字面）。
 * ⚠️ **那道 gate 掃的是 `.astro` 字面**，擋的是「有人又把這種句子寫死進頁面」。
 *    如果是**資料層**新長出一種資料集級宣稱而沒有加進下面這個清單，兩層都不會紅燈——
 *    它會安靜地出現在每一頁。定期查法：把資料層無 url 的 ref 按出現次數排序，
 *    重複到成千上萬次的那幾筆就是候選。
 */
export const DATASET_LEVEL_CLAIMS = [
  '內政部全國宗教資訊網（data.gov.tw/dataset/8203）',
  '交通部觀光署觀光資訊資料庫・景點（政府資料開放平臺 dataset 7777）',
] as const;

/** 這一筆是不是「資料集級整包宣稱」（＝不該渲染到頁面上）。比對 ref 全文，不做模糊比對。 */
export function isDatasetLevelClaim(s: SourceRef): boolean {
  const ref = (s.ref ?? '').trim();
  return (DATASET_LEVEL_CLAIMS as readonly string[]).includes(ref);
}
